...
void ASWeapon::Fire()
{
	...
	{	
		...
		FVector ShotDirection = EyeRotation.Vector(); //방향
		...
		QueryParams.AddIgnoredActor(MyOwner); // 나
		QueryParams.AddIgnoredActor(this); // 무기
		...
		if (GetWorld()->LineTraceSingleByChannel(Hit, EyeLocation, TraceEnd, COLLISION_WEAPON, QueryParams))
		{
			...
			AActor* HitActor = Hit.GetActor(); //맞은 액터
			...
			//ApplyPointDamage(맞은 액터, 대미지, 방향, 히트결과, 가해자, 대미지 이유, 대미지 타입)
...
void ASWeapon::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(ASWeapon, HitScanTeace, COND_SkipOwner); // 무기를 쏜 사람 제외
}